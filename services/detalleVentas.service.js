...
async function getBestSellingProductsService({ start, end }) {
  const query =
    !start && !end
      ? 'select dv.id_prod as "idProd",p.nombre,c.nombre as categoria,m.nombre as marca, pr.nombre as proveedor,  SUM(dv.cantidad) as "cantidadVendida" from "Detalle_Ventas" as dv, "Productos" as p, "Categorias" as c, "Marcas" as m, "Proveedores" as pr where p.id = dv.id_prod and p.id_cat= c.id and p.id_marca= m.id and p.id_prov = pr.id  GROUP BY id_prod,p.nombre,c.nombre, m.nombre, pr.nombre ORDER BY "cantidadVendida" DESC;'
      : `select dv.id_prod as "idProd",p.nombre,c.nombre as categoria,m.nombre as marca, pr.nombre as proveedor, v.fecha, SUM(dv.cantidad) as "cantidadVendida" from "Detalle_Ventas" as dv, "Productos" as p, "Categorias" as c, "Marcas" as m, "Proveedores" as pr, "Ventas" as v where p.id = dv.id_prod and p.id_cat= c.id and p.id_marca= m.id and p.id_prov = pr.id and v.id= dv.id_venta and  v.fecha between '${start}' and '${end}' GROUP BY id_prod,p.nombre,c.nombre, m.nombre, pr.nombre, v.fecha ORDER BY "cantidadVendida" DESC;`
